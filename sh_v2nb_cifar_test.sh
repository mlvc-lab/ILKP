    ...
    do
        ...
        do
            if [ "$model" = "resnet" ]; then
                ...
                python3 main.py $data -a $model -j 4 -C -g 0 -E -N --ckpt "ckpt_best_v2nb_pwd8_pws1_q"$qb"8.pth" --qb $qb
                python3 main.py $data -a $model -j 4 -C -g 0 1 -b 256 -T --ckpt "ckpt_best_v2nb_pwd8_pws1_q"$qb"8.pth" -N -v v2nb --qb $qb --lr 0.01
                rm -f checkpoint/*/*/ckpt_rt*_v*_pwd8_pws1_q*_s*_epoch_*.pth
                python3 main.py $data -a $model -j 4 -C -g 0 -E -N --ckpt "ckpt_rt1_v2nb_pwd8_pws1_q"$qb"8_s5_best.pth" --qb $qb
            fi
        done
    done
done
