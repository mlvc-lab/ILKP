...
class ShuffleNet(nn.Module):
    ...
    def get_layer_dwconv(self, layer_num=0):
        if layer_num < len(self.stage2):
            return self.stage2[layer_num].depthwise_conv3x3
        elif layer_num < len(self.stage2) + len(self.stage3):
            return self.stage3[layer_num-len(self.stage2)].depthwise_conv3x3
        elif layer_num < self.get_num_dwconv_layer():
            return self.stage4[layer_num-len(self.stage2)-len(self.stage3)].depthwise_conv3x3
        else:
    ...
    def get_weights_dwconv(self, use_cuda=True):
        ...
        for i in range(len(self.stage2)):
            if use_cuda:
                _w_dwconv = self.stage2[i].depthwise_conv3x3.weight.cpu()
            else:
                _w_dwconv = self.stage2[i].depthwise_conv3x3.weight
            _w_dwconv = _w_dwconv.detach().numpy()
            w_dwconv.append(_w_dwconv)
        for i in range(len(self.stage3)):
            if use_cuda:
                _w_dwconv = self.stage3[i].depthwise_conv3x3.weight.cpu()
            else:
                _w_dwconv = self.stage3[i].depthwise_conv3x3.weight
            _w_dwconv = _w_dwconv.detach().numpy()
            w_dwconv.append(_w_dwconv)
        for i in range(len(self.stage4)):
            if use_cuda:
                _w_dwconv = self.stage4[i].depthwise_conv3x3.weight.cpu()
            else:
                _w_dwconv = self.stage4[i].depthwise_conv3x3.weight
            _w_dwconv = _w_dwconv.detach().numpy()
            w_dwconv.append(_w_dwconv)
        return w_dwconv
    ...
    def set_weights_dwconv(self, weight, use_cuda=True):
        if use_cuda:
            gpuid = self.stage2[0].depthwise_conv3x3.weight.get_device()
        ...
        for i in range(len(self.stage2)):
            ...
            self.stage2[i].depthwise_conv3x3.weight.data.copy_(weight_tensor)
        for i in range(len(self.stage3)):
            k = i + len(self.stage2)
            if use_cuda:
            ...
            self.stage3[i].depthwise_conv3x3.weight.data.copy_(weight_tensor)
        for i in range(len(self.stage4)):
            k = i + len(self.stage2) + len(self.stage3)
            if use_cuda:
                weight_tensor = torch.from_numpy(weight[k]).float().to(device)
            else:
                weight_tensor = torch.from_numpy(weight[k]).float()
            self.stage4[i].depthwise_conv3x3.weight.data.copy_(weight_tensor)
...
class ShuffleNet_CIFAR(nn.Module):
    ...
    def __init__(self, groups=3, in_channels=3, width_mult=1.0, num_classes=1000):
        ...
        Arguments:
            ...
            num_classes (int, optional): number of classes to predict. Default
                is 1000 for ImageNet.
        """
        super(ShuffleNet_CIFAR, self).__init__()
