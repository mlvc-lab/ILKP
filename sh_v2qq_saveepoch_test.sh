        ...
        do
            ...
            do
                if [ "$model" = "resnet" ]; then
                    ...
                    python3 main.py $data -a $model -j 4 -C -g 0 1 -b 256 -T --ckpt "ckpt_best_v2qq_q"$qb"88.pth" -N -v v2qq --qb $qb --lr 0.01 -s $save
                    rm -f checkpoint/*/*/ckpt_rt*_v*_q*_s*_epoch_*.pth
                    python3 main.py $data -a $model -j 4 -C -g 0 -E -N --ckpt "ckpt_rt1_v2qq_q"$qb"88_s"$save"_best.pth" --qb $qb
                fi
            done
        done
    done
