        ...
        do
            if [ "$model" = "resnet" ]; then
                ...
                do
                    ...
                    python3 main.py $data -a $model --layers $layer -j 4 -C -g 0 -E --ckpt "ckpt_best_q"$qb".pth"
                    python3 main.py $data -a $model --layers $layer -j 4 -C -g 0 1 -T -Q --qb $qb -b 256 --ckpt "ckpt_best_q"$qb".pth" --lr 0.01
                    rm -f checkpoint/*/*/ckpt_rt*_q*_epoch_*.pth
                    python3 main.py $data -a $model --layers $layer -j 4 -C -g 0 -E --ckpt "ckpt_rt1_q"$qb"_best.pth"
                ...
                python3 main.py $data -a $model -j 4 -C -g 0 -E --ckpt "ckpt_best_q"$qb".pth"
                python3 main.py $data -a $model -j 4 -C -g 0 1 -T -Q --qb $qb -b 256 --ckpt "ckpt_best_q"$qb".pth" --lr 0.01
                rm -f checkpoint/*/*/ckpt_rt*_q*_epoch_*.pth
                python3 main.py $data -a $model -j 4 -C -g 0 -E --ckpt "ckpt_rt1_q"$qb"_best.pth"
                echo "linear quantization-ifl baseline"
