        ...
        do
            if [ "$model" = "resnet" ]; then
                ...
                do
                    ...
                    python3 main.py $data -a $model --layers $layer -j 2 -C -g 0 -E -N --ckpt "ckpt_new_v2qq_q"$qb"a8b8_s1_best.pth" --qb $qb
                done
            else
                python3 main.py $data -a $model -j 2 -C -g 0 1 2 3 -b 256 -N -v v2qq --qb $qb
                rm -f checkpoint/*/*/ckpt_new_v*_q*_s*_epoch_*.pth
                python3 main.py $data -a $model -j 2 -C -g 0 -E -N --ckpt "ckpt_new_v2qq_pwd8_pws1_q"$qb"a8b8_s1_best.pth" --qb $qb
            fi
        done
    done
done
